...
# --- the analysis suites' factorial ----------------------------------------

STATE_CONDITION = {"base": "base", "abliterated": "layer_23"}


def analysis_scores(unlocked=None, malformed=None, prompt_ids=(0, 1, 2)) -> list:
    """A whole factorial at the draw schedule, built through the reader's own derivation.

    ``unlocked`` and ``malformed`` map ``(state, level)`` to one count per prompt; an
    unnamed cell is all refusals and nothing malformed. Within a level the unlocked draws
    are taken from the front and the malformed ones from the back, so the two never land
    on the same row -- a malformed grade is a non-unlock. Which slot of a family a draw
    falls in is not something a family-level count can distinguish anyway.
    """
    from analysis.read import derived
    from harness.dataset import CONTROL
    from inspect_ai.scorer import SampleScore, Score
    from prefills import PORTFOLIO, family_of
    from study import draws

    unlocked, malformed = unlocked or {}, malformed or {}
    by_level: dict[str, list[str]] = {}
    for slot in (CONTROL, *PORTFOLIO):
        by_level.setdefault(family_of(slot), []).append(slot)

    out = []
    for state, condition in STATE_CONDITION.items():
        for index, prompt_id in enumerate(prompt_ids):
            for level, slots in by_level.items():
                cells = [(slot, draw) for slot in slots for draw in range(draws(slot))]
                n_u = unlocked.get((state, level), [0] * len(prompt_ids))[index]
                n_m = malformed.get((state, level), [0] * len(prompt_ids))[index]
                for position, (slot, draw) in enumerate(cells):
                    value = {
                        "unlocked": int(position < n_u),
                        "malformed": int(position >= len(cells) - n_m),
                    }
                    out.append(
                        SampleScore(
