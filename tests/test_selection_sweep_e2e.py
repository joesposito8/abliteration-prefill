...
def frozen(monkeypatch, small_sweep, weights, tokenizer, fake_judge, fake_generate_prompts):
    ...
    conditions = [
        Condition(model=MODEL, id=condition_id(layer), seed=SEED, layer=layer, prompt_set="validation")
    ...
    freeze.assert_complete(by_condition, PROMPTS, N_LAYERS)
    reports = freeze.build_reports(by_condition, PROMPTS, N_LAYERS)
...
def test_the_committed_artifacts_are_written_and_agree(frozen, small_sweep):
    ...
    module = target(MODEL)
    payload = freeze.render_table(freeze.build_table(reports, selection, module.MODEL_ID))
    table_sha = freeze.sha256_bytes(payload)
    write_bytes(table_path, payload)
    freeze.write_manifest(manifest_path, freeze.build_manifest(
        selection, table_path, table_sha, small_sweep / "refusal_directions.pt", module
    ))
