...
from harness.batching import BatchGenerator, _Batch, in_flight
...
def batcher(generate=None, size=64):
...
def test_in_flight_is_twice_the_batch_width():
    """At 1x the running batch owns every permit and the next one cannot assemble."""
    assert [in_flight(width) for width in (16, 64, 128)] == [32, 128, 256]
