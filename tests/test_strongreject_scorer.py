...
def generated(tmp_path, tokenizer, prefills, sample_id: str):
    ...
    condition = Condition(model=MODEL, 
        id="layer_22", seed=SEED, layer=22, prompt_set="strongreject", num_prompts=30, prefilled=True
    )
    return eval(
        refusal_unlock(build_dataset(condition, prefills), seed=SEED, model=condition.model),
