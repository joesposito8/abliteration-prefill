...
def test_a_thirty_prompt_run_is_the_committed_pilot_slice():
    """What retiring the pilot prompt set rests on: ``--num-prompts 30`` over the 313 draws the
    same 30 rows the frozen CSV holds, so the archived run is still reproducible."""
    condition = Condition(
        model="qwen3-4b", id="layer_23", seed=SEED, layer=23,
        prompt_set="strongreject", num_prompts=30,
    )
    ids = sorted({s.metadata["prompt_id"] for s in build_dataset(condition, {})})

    assert ids == load_pilot_prompts()["prompt_id"].tolist()
