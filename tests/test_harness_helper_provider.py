...
from generation.gemma3_27b import HELPER_DECODING
...
from harness.provider import LocalTargetAPI
...
@pytest.mark.parametrize(
    "api, model_name, width",
    [
        pytest.param(GemmaHelperAPI, "prefills", HELPER_BATCH, id="helper"),
        pytest.param(LocalTargetAPI, "qwen3-4b/base", qwen3_4b.BATCH, id="target"),
    ],
)
def test_a_provider_admits_at_least_its_own_batch_width(api, model_name, width):
    """A limiter is sized by whichever call creates it first, so anything below the
    width deadlocks a batch that can never fill."""
    assert api(model_name=model_name).max_connections() >= width
...
def test_the_prompt_opens_the_models_turn(helper_tokenizer, fake_helper_prompts):
    ...
    assert call.request["prompt"].endswith(gemma3_27b.TURN_SUFFIX)
