...
from harness.batching import in_flight
from harness.conditions import PROVIDER
from harness.provider import LocalTargetAPI, frozen_config, split_prefill
...
FROZEN_CONFIG = frozen_config(qwen3_4b)
IN_FLIGHT = in_flight(qwen3_4b.BATCH)


def build(tokenizer=None, module=None) -> LocalTargetAPI:
    # What Inspect passes: the provider half is already stripped by get_model.
    return LocalTargetAPI(
        model_name="qwen3-4b/layer_22", module=module, tokenizer=tokenizer
    )
...
async def generate(api: LocalTargetAPI, messages, cfg):
...
def test_a_misspelled_model_arg_is_refused(tokenizer):
    ...
    with pytest.raises(TypeError, match="toknizer"):
        get_model("local/qwen3-4b/typo-check", toknizer=tokenizer)
...
def test_the_entry_point_registers_the_provider_and_defers_torch():
    ...
    model = get_model(
        "local/qwen3-4b/base", config=GenerateConfig(max_connections=IN_FLIGHT)
    )
    assert isinstance(model.api, LocalTargetAPI)
    assert model.api.max_connections() == IN_FLIGHT


def test_every_enabled_target_resolves_through_the_registry():
    """The registry name is a literal in a decorator and ``PROVIDER`` is a constant in
    another file; this is the only thing holding the two together."""
    from generation import TARGETS
    from study.datasets import model_slug

    for module in TARGETS:
        api = get_model(
            f"{PROVIDER}/{model_slug(module.MODEL_ID)}/base",
            config=GenerateConfig(max_connections=in_flight(module.BATCH)),
        ).api
        assert api.model is module


def test_a_model_name_without_a_condition_is_refused():
    """Every condition of one target would otherwise be the same run to eval_set."""
    with pytest.raises(ValueError, match="no condition"):
        LocalTargetAPI(model_name="qwen3-4b")
...
def test_a_thinking_leak_is_flagged(tokenizer, monkeypatch):
    """The shape the pilot's leaks took: no opening tag, prose, a bare closing tag. The
    markers survive skip_special_tokens, so a leak is detectable but not visible."""
...
def test_model_call_records_the_rendered_prompt(tokenizer, fake_generate_prompts):
    ...
    from generation.qwen3_4b import TURN_SUFFIX
    ...
    assert call.request["prompt"].endswith(TURN_SUFFIX + "Sure:")
    assert call.request["seed"] == SEED
