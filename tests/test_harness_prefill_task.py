...
import pytest
from generation.gemma import render_prompt
...
def test_the_task_records_what_the_wave_was_produced_under():
    ...
    assert len(task.metadata["portfolio_sha256"]) == 64
...
def test_the_helper_prompt_reaches_the_model_byte_for_byte(
    ...
    """The six helper prompts are authored files hashed into portfolio_sha256. A byte
    altered on the way to the GPU means the prefills were produced under a prompt that
    is not the frozen one, with the log recording the frozen one's hash regardless."""
