"""Target model: Qwen3-4B with thinking disabled, optionally prefilled.

What this module holds is everything that is this checkpoint's own. The batched forward
pass is model-agnostic and lives in :mod:`generation.batched`; the decoding parameters
are study-wide and live in :mod:`generation.decoding`.

Two properties here fail silently rather than loudly, so each is enforced in code
...
# Every leak measured so far was a bare closing tag, so both halves are named.
