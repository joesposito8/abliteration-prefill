...
def main() -> None:
    ...
    def stage(path: Path, frame: pd.DataFrame) -> str:
    ...
    artifacts[HARMBENCH_STANDARD_CSV.name] = {
        "sha256": stage(HARMBENCH_STANDARD_CSV, behaviors[COLS_HARMFUL]),
        "rows": len(behaviors),
    }
    ...
    artifacts[EXTRACTION_HARMFUL_CSV.name] = {
        "sha256": stage(EXTRACTION_HARMFUL_CSV, ext_df[COLS_HARMFUL]),
        "rows": len(ext_df),
    }
    artifacts[VALIDATION_HARMFUL_CSV.name] = {
        "sha256": stage(VALIDATION_HARMFUL_CSV, val_df[COLS_HARMFUL]),
        "rows": len(val_df),
    }
    ...
    artifacts[EXTRACTION_HARMLESS_CSV.name] = {
        "sha256": stage(EXTRACTION_HARMLESS_CSV, harmless_df[["prompt", "alpaca_index"]]),
    ...
    artifacts[PILOT_PROMPTS_CSV.name] = {
        "sha256": stage(PILOT_PROMPTS_CSV, pilot_df[cols_pilot]),
        "rows": len(pilot_df),
    }
