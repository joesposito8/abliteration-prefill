...
def grade_log(source: Path, output: Path) -> EvalLog:
    """Score one condition, whole. The largest is 50,080 samples and ~5 GB read in.
...
def grade_sweep(source_root: Path, output_root: Path) -> list[EvalLog]:
    ...
    for log in finished_logs(source_root):
        ...
        grade_log(source, output)
        # Whole logs here would hold every condition of the sweep resident at once.
        scored.append(read_eval_log(str(output), header_only=True))
    return scored
