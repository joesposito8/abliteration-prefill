...
from study.datasets import REFUSAL_DIRECTIONS_PT  # noqa: E402
...
def main(argv: list[str]) -> None:
    ...
    if not REFUSAL_DIRECTIONS_PT.exists():
        raise SystemExit(f"{REFUSAL_DIRECTIONS_PT} is missing; run scripts/extract_directions.py")

    directions = load_directions(REFUSAL_DIRECTIONS_PT)
