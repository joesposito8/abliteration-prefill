...
from generation.gemma3_27b import HELPER_DECODING  # noqa: E402
...
def build_manifest(table: pd.DataFrame, csv_sha256: str) -> dict:
    ...
    spec = {
        ...
        "inputs": {path.name: sha256_file(path) for path in (STRONGREJECT_CSV,)},
    }
    manifest = dict(spec)
...
def main(argv: list[str]) -> None:
    ...
    payload = render_csv(table)
    csv_sha256 = sha256_bytes(payload)
    write_bytes(PREFILLS_CSV, payload)
