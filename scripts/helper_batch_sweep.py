...
from generation.gemma3_27b import HELPER_DECODING, build_prompt, load_helper  # noqa: E402
...
from batch_sweep import kv_bytes_per_token  # noqa: E402
...
def workload(tokenizer, rows: int) -> list[str]:
    ...
    return [
        build_prompt(tokenizer, fill_prompt(templates[index % len(templates)], prompt))
...
def main(argv: list[str]) -> None:
    ...
    winner = next(w for w in widths if w["width"] == choice.width)
    ...
    print(f"\nSet HELPER_BATCH = {choice.width} in src/harness/helper_provider.py before "
          "running the waves.")
